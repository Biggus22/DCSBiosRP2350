...
// --- CRC-8 table: PROGMEM on AVR (saves 256B from 2KB RAM), RAM elsewhere ---
#ifdef __AVR__
#include <avr/pgmspace.h>
#define CRC8_TABLE_STORAGE PROGMEM
#else
#define CRC8_TABLE_STORAGE
#endif

// --- CRC-8 (poly 0x07, custom, not Dallas/Maxim 1-Wire) ---
static const uint8_t crc8_table[256] CRC8_TABLE_STORAGE = {
...
static inline uint8_t i2cFrame_crc8(const uint8_t *data, uint8_t len) {
    if (!data) return 0xFF;
    uint8_t crc = 0;
    for (uint8_t i = 0; i < len; i++) {
        crc = CRC8_TABLE_READ(crc ^ data[i]);
    }
    return crc;
}
