...
const uint8_t pltAfcsAltHoldPins[2] = {8, 9};
DcsBios::SwitchMultiPos<3> pltAfcsAltHold("PLT_AFCS_ALT_HOLD", pltAfcsAltHoldPins, true);
const uint8_t pltAfcsAutopilotPins[2] = {12, 11};
DcsBios::SwitchMultiPos<3> pltAfcsAutopilot("PLT_AFCS_AUTOPILOT", pltAfcsAutopilotPins, true);

// On-off toggle for magnetically held momentary toggles
/*const uint8_t pltAfcsAltHoldPin = 8;
DcsBios::Switch2Pos pltAfcsAltHold("PLT_AFCS_ALT_HOLD", pltAfcsAltHoldPin);
const uint8_t pltAfcsAutopilotPin = 12;
DcsBios::Switch2Pos pltAfcsAutopilot("PLT_AFCS_AUTOPILOT", pltAfcsAutopilotPin);
*/
const uint8_t pltAfcsStabAugPitchPins[2] = {15, 13};
DcsBios::SwitchMultiPos<2> pltAfcsStabAugPitch("PLT_AFCS_STAB_AUG_PITCH", pltAfcsStabAugPitchPins);
const uint8_t pltAfcsStabAugRollPins[2] = {27, 26};
DcsBios::SwitchMultiPos<2> pltAfcsStabAugRoll("PLT_AFCS_STAB_AUG_ROLL", pltAfcsStabAugRollPins);
const uint8_t pltAfcsStabAugYawPins[2] = {28, 29};
DcsBios::SwitchMultiPos<2> pltAfcsStabAugYaw("PLT_AFCS_STAB_AUG_YAW", pltAfcsStabAugYawPins);

// DCS-BIOS F-14A/B FUNCTIONS HERE
const uint8_t pltAutopltAltPins[2] = {8, 9};
DcsBios::SwitchMultiPos<3> pltAutopltAlt("PLT_AUTOPLT_ALT", pltAutopltAltPins, true);
const uint8_t pltAutopltEngagePins[2] = {12, 11};
DcsBios::SwitchMultiPos<3> pltAutopltEngage("PLT_AUTOPLT_ENGAGE", pltAutopltEngagePins, true);
const uint8_t pltAfcsPitchPins[2] = {15, 13};
DcsBios::SwitchMultiPos<2> pltAfcsPitch("PLT_AFCS_PITCH", pltAfcsPitchPins);
const uint8_t pltAfcsRollPins[2] = {27, 26};
DcsBios::SwitchMultiPos<2> pltAfcsRoll("PLT_AFCS_ROLL", pltAfcsRollPins);
const uint8_t pltAfcsYawPins[2] = {28, 29};
DcsBios::SwitchMultiPos<2> pltAfcsYaw("PLT_AFCS_YAW", pltAfcsYawPins);
...
void onF14PltIntLightConsoleChange(unsigned int consoleBrightness)
{
    ...
    {
        // If it's a 16-bit value, convert normally
...
int main()
{
    ...
    setup_pwm(altHoldCoilPin); // Setup PWM for Alt Hold coil
