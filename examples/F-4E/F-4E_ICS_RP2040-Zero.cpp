...
DcsBios::SwitchMultiPos<3> pltIcsAmplifier("PLT_ICS_AMPLIFIER", pltIcsAmplifierPins);
const uint8_t pltIcsModePins[3] = {8, 9};
DcsBios::SwitchMultiPos<3> pltIcsMode("PLT_ICS_MODE", pltIcsModePins);
...
void onPltIntLightConsoleChange(unsigned int consoleBrightness)
{
    ...
    {
        externalLeds.setPixel(i, externalLeds.rgbw(0, intensity, 0, 0));
    }
    externalLeds.show();
}
